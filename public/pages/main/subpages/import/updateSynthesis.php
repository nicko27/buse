...
try {
    ...
    $fileInDirectory = $excelProcessor->getXlsxFilesInDirectory($config->get("UPLOAD_DIR"));
    foreach ($fileInDirectory as $file) {
        $excelProcessor->processXlsxFile($file);
        $totalCols    = explode("|", $config->get("COL_LIST"));
        $colsToSearch = [];
        foreach ($totalCols as $col) {
            $colsToSearch[$col] = $config->get($col);
        }
        $cols = $excelProcessor->getColsCoordonates($colsToSearch);
