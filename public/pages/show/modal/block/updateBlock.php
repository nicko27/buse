...
try {
    ...
    $sql  = "SELECT * FROM services_unites WHERE id = :id";
    $stmt = $sqlManager->prepare($sql);
    $stmt->execute(['id' => $id]);
    $existingBlock = $stmt->fetch();

    if (!$existingBlock) {
        throw new Exception("Bloc non trouvé avec l'ID: " . $id);
    }

    $logger->info("Bloc existant:", $existingBlock);
    ...
    $logger->info("Données pour mise à jour:", $data);
    $logger->info("Condition de mise à jour: id=" . $id);

    // Mise à jour dans la base de données
    $result = $sqlManager->update("services_unites", $data, "id=" . $id);
    $logger->info("Résultat de la mise à jour:", $result);
