...
include_once "fonctions/getTimelineBlocksFcts.php";
...
try {
    ...
    WHERE services_unites.invisible = 0 and services.invisible = 0
    ...
    foreach ($results as $resultat) {
        ...
        if ($entry_fin_time >= $first_case_time) {
            ...
            if (($fin_index >= 1 || $debut_index >= 1) && ($debut_index <= $nb_quart_heure)) {
                ...
                $cu                       = $resultat["cu"];
                if ($resultat['cu'] == 0) {
                    $sqlur  = "select to_cu from unit_redirection where from_cu=:from_cu";
                    $stmtur = $sqlManager->prepare($sqlur);
                    $stmtur->execute([":from_cu" => $resultat["cu_cob"]]);
                    $resultur = $stmtur->fetch();
    ...
    foreach ($cuList as $cu) {
        ...
        $blocks = handleBlocksByCu($listeResult[$cu], $config->get("NB_QUART_HEURE"), $config->get("MINIMUM_BLOCK_CONCAT"), $config->get("NB_LINES_TIMELINE"));
        foreach ($blocks as $bloc) {
            ...
            $bloc["divContent"]      = $twig->render('/show/block_timeline.twig', $bloc);
            $newBlocks[]             = $bloc;
